...
## 2026-08-19 — 앨범 카드 · 수록곡 모달

- 한 일: 벽·필터·셔플·인덱스·수집의 기본 단위를 곡에서 앨범으로 전환.
  앨범 카드를 열면 대표 곡을 포함한 4~7개 수록곡, 총 재생시간, 레이블을 보여주는
  상세 모달이 열린다. 새로고침마다 세션 시드를 바꿔 새 앨범·수록곡·커버아트를
  만들고, 수집한 앨범은 당시 시드로 복원한다.
